...
# Build the container:
# docker build . -t gaussiandream_server -f scripts/docker/serve_policy.Dockerfile

# Run the container:
# docker run --rm -it --network=host -v .:/app --gpus=all gaussiandream_server /bin/bash
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    --mount=type=bind,source=gaussiandream-client/pyproject.toml,target=gaussiandream-client/pyproject.toml \
    --mount=type=bind,source=gaussiandream-client/src,target=gaussiandream-client/src \
    GIT_LFS_SKIP_SMUDGE=1 uv sync --frozen --no-install-project --no-dev

# Copy transformers_replace files while preserving directory structure
COPY src/gaussiandream/models_pytorch/transformers_replace/ /tmp/transformers_replace/
