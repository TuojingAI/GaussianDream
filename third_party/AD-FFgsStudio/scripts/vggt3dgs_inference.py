...
def render_novel_views(model, recontrast_data, render_data, novel_render_frames, device, scene_name, sample_idx, save_dir, actual_sample_idx, translation_distances=[1.0, 2.0], eval_resolution='280x518'):
    ...
    for transform_name, transform_matrix in translation_transforms.items():
        ...
        for novel_frame_id in novel_render_frames:
            for cam_id in range(num_cams):
                # Get original camera extrinsics and intrinsics
                original_e2c_extr = render_data[novel_frame_id][('e2c_extr', cam_id)][sample_idx:sample_idx+1]
                K_i = render_data[novel_frame_id][('K', cam_id)][sample_idx:sample_idx+1, :3, :3]
                ...
                # Save the novel view
                global_sample_idx = actual_sample_idx + novel_frame_id
...
def _process_scene_batch(model, scene_batch, device, gpu_id=0, save_renders=True, output_dir=None, novel_distances=[1.0, 2.0], eval_resolution='280x518',eval_frame=None, frame_skip=None):
    ...
    for batch in scene_loader:
        ...
        try:
            output = model.predict_step(batch_gpu, novel_render_frames)
            model_width = getattr(model, 'width', 518)
            model_height = getattr(model, 'height', 280)
