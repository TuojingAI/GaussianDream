...
def run_inference(model_cfg=None, model=None, checkpoint_path=None, 
                  ...
                  eval_resolution='280x518',eval_frame=None, frame_skip=None):
    """
    ...
    if frame_skip is not None:
        print(f'frame_skip: {frame_skip} (evaluating every {frame_skip} frames)')
    ...
    if not is_multi_gpu:
        ...
        return _run_single_gpu_inference(model, scene_dataloader, device, save_results, output_dir, novel_distances, eval_resolution,eval_frame, frame_skip)
    ...
    # Multi-GPU path  
        ...
        return _run_multi_gpu_inference(model_cfg, scene_dataloader, device_list, checkpoint_path, save_results, output_dir, novel_distances, eval_resolution,eval_frame, frame_skip)
...
def render_novel_views(model, output_data, novel_render_frames, device, scene_name, sample_idx, save_dir, actual_sample_idx, translation_distances=[1.0, 2.0], eval_resolution='280x518'):
    ...
    for transform_name, transform_matrix in translation_transforms.items():
        ...
        for frame_id in novel_render_frames:
            for cam_id in range(num_cams):
                ...
                xyz_i = output_data[('xyz',frame_id,cam_id)][batch_id].to(torch.float32)
                rot_i = output_data[('rot',frame_id,cam_id)][batch_id].to(torch.float32)
                scale_i = output_data[('scale',frame_id,cam_id)][batch_id].to(torch.float32)
                opacity_i = output_data[('opacity',frame_id,cam_id)][batch_id].to(torch.float32)
                sh_i = output_data[('sh',frame_id,cam_id)][batch_id].to(torch.float32)

                original_e2c_extr_i = output_data[('e2c_extr', frame_id)][batch_id:batch_id+1,cam_id].to(torch.float32)
                K_i = output_data['K'][batch_id:batch_id+1,cam_id,:3,:3].to(torch.float32)
                
                # Apply lateral translation to camera pose
                # Transform ego to camera: new_e2c = e2c @ inv(transform)
                novel_e2c_extr = torch.matmul(original_e2c_extr_i, torch.linalg.inv(transform_matrix.unsqueeze(0)))
...
def _process_scene_batch(model, scene_batch, device, gpu_id=0, save_renders=True, output_dir=None, novel_distances=[1.0, 2.0], eval_resolution='280x518',eval_frame=None, frame_skip=None):
    ...
    for batch in scene_loader:
        ...
        try:
            output_data = model.predict_step(batch_gpu, [0,6], novel_render_frames)
            model_width = getattr(model, 'width', 518)
            model_height = getattr(model, 'height', 280)
            ...
            for frame_id in novel_render_frames:
                ...
                for cam_id in range(num_cams):
                    ...
                    if pred_key in output_data and gt_key in output_data:
                        pred = output_data[pred_key][0:1]
                        gt = output_data[gt_key][0:1]
            ...
            if save_renders:
                try:
                    novel_view_paths = render_novel_views(
                        model, output_data, novel_render_frames,
...
def _run_single_gpu_inference(model, scene_dataloader, device, save_results=True, output_dir=None, novel_distances=[1.0, 2.0], eval_resolution='280x518',eval_frame=None, frame_skip=None):
    ...
    with torch.no_grad():
        for scene_idx, scene_batch in enumerate(scene_dataloader):
            ...
            result = _process_scene_batch(model, scene_batch, device, gpu_id=0, 
                                        save_renders=save_results, output_dir=output_dir, novel_distances=novel_distances, eval_resolution=eval_resolution,eval_frame=eval_frame, frame_skip=frame_skip)
...
def run_single_gpu_inference(gpu_id, scene_indices, all_scenes, config, checkpoint_path, save_results, output_dir, results_queue, novel_distances=[1.0, 2.0], eval_resolution='280x518',eval_frame=None, frame_skip=None):
    """
    ...
    Args:
        ...
        novel_distances: List of distances for novel view generation
        eval_resolution: Resolution mode ('original' or 'upsampled')
        frame_skip: Skip interval for frames (e.g., 6 to evaluate every 6th frame)
    """
    try:
...
def _run_multi_gpu_inference(model_cfg, scene_dataloader, devices, checkpoint_path, save_results=True, output_dir=None, novel_distances=[1.0, 2.0], eval_resolution='280x518',eval_frame=None, frame_skip=None):
    """Run inference on multiple GPUs with scene distribution"""
    print(f"\nStarting multi-GPU scene-based inference")
    print(f"Devices: {devices}")
    if frame_skip is not None:
        print(f"Frame skip: {frame_skip} (evaluating every {frame_skip} frames)")
    print(f"Number of scenes to process: {len(scene_dataloader)}")
    ...
    for device_id, scene_indices in scene_distribution:
        if len(scene_indices) > 0:  # Only start thread if there are scenes to process
            thread = threading.Thread(
                ...
                args=(device_id, scene_indices, all_scenes, {'model_cfg': model_cfg}, checkpoint_path, save_results, output_dir, results_queue, novel_distances, eval_resolution, eval_frame, frame_skip)
            )
            thread.start()
...
def main():
    ...
    parser.add_argument('--frame_skip', type=int, default=None, 
                       help='Skip interval for frames (e.g., 6 to evaluate frames 0, 6, 12, 18, ...)')
    ...
    # Print frame skip information
    if args.frame_skip is not None:
        print(f"Frame skip: {args.frame_skip} (evaluating frames 0, {args.frame_skip}, {2*args.frame_skip}, ...)")
