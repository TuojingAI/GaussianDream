...
class DepthNetwork(nn.Module):
    ...
    def forward(self, inputs,MF_frames=[-1,1]):
        '''
        ...
        sf_images = torch.stack([inputs[('color_aug', 0)][:, cam, ...] for cam in range(self.num_cams)], 1) 

        batch_size = sf_images.shape[0]
        if self.novel_view_mode == 'MF':
            sf_images_last = torch.stack([inputs[('color_aug', MF_frames[0])][:, cam, ...] for cam in range(self.num_cams)], 1)
            sf_images_next = torch.stack([inputs[('color_aug', MF_frames[0])][:, cam, ...] for cam in range(self.num_cams)], 1)
        ...
        feats_agg = unpack_cam_feat(packed_feats_agg, batch_size, self.num_cams) 
        if self.novel_view_mode == 'MF':
            feats_agg_last = unpack_cam_feat(packed_feats_agg_last, batch_size, self.num_cams)
            feats_agg_next = unpack_cam_feat(packed_feats_agg_next, batch_size, self.num_cams)
        ...
        for i in range(len(feat_in)):
            img_feat.append(unpack_cam_feat(feat_in[i], batch_size, self.num_cams))
        
        if self.novel_view_mode == 'MF':
            feat_in_last = packed_feats_last[:lev] + [fusion_dict_last['proj_feat']] 
            img_feat_last = []
            for i in range(len(feat_in_last)):
                img_feat_last.append(unpack_cam_feat(feat_in_last[i], batch_size, self.num_cams))
            
            feat_in_next = packed_feats_next[:lev] + [fusion_dict_next['proj_feat']] 
            img_feat_next = []
            for i in range(len(feat_in_next)):
                img_feat_next.append(unpack_cam_feat(feat_in_next[i], batch_size, self.num_cams))
        ...
        depth_outputs = unpack_cam_feat(packed_depth_outputs, batch_size, self.num_cams) 
        if self.novel_view_mode == 'MF':
            depth_outputs_last = unpack_cam_feat(packed_depth_outputs_last, batch_size, self.num_cams)
            depth_outputs_next = unpack_cam_feat(packed_depth_outputs_next, batch_size, self.num_cams)
        ...
        for cam in range(self.num_cams):
            ...
            outputs[('cam', cam)][('disp', 0, 0)] = depth_outputs[('disp', 0)][:, cam, ...]  
            if self.novel_view_mode == 'MF':
                outputs[('cam', cam)][('img_feat', MF_frames[0], 0)] = [feat[:, cam, ...] for feat in img_feat_last] 
                outputs[('cam', cam)][('img_feat', MF_frames[1], 0)] = [feat[:, cam, ...] for feat in img_feat_next]
                outputs[('cam', cam)][('disp', MF_frames[0], 0)] = depth_outputs_last[('disp', 0)][:, cam, ...] 
                outputs[('cam', cam)][('disp', MF_frames[1], 0)] = depth_outputs_next[('disp', 0)][:, cam, ...] 
