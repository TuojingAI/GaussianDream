...
from kornia.geometry.conversions import angle_axis_to_rotation_matrix as axis_angle_to_matrix
...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        ones = torch.ones(1, 1, self.n_voxels)
        # Using register_buffer to ensure they move with the model
        self.register_buffer('voxel_pts', torch.cat([voxel_grid.view(b, 3, self.n_voxels), ones], dim=1))
        ...
        self.register_buffer('pixel_grid', self.create_pixel_grid(1, self.img_h, self.img_w))
        self.register_buffer('pixel_ones', torch.ones(1, 1, self.proj_d_bins, self.num_pix))
        
        # define a depth grid for projection
        depth_bins = torch.linspace(self.proj_d_str, self.proj_d_end, self.proj_d_bins)
        self.register_buffer('depth_grid', self.create_depth_grid(1, self.num_pix, self.proj_d_bins, depth_bins))
    ...
    def create_voxel_grid(self, str_p, end_p, v_size):
        ...
        grids = [grid.expand(1, 1, z_dim, y_dim, x_dim) for grid in grids]
    ...
    def create_depth_grid(self, batch_size, n_pixels, n_depth_bins, depth_bins):
        ...
        # expand creates a view with 0-stride, which is not contiguous and causes issues with safetensors saving/loading
        depth_layers = depth_layers.expand(batch_size, 3, n_depth_bins, n_pixels).contiguous()
        return depth_layers

    def type_check(self, sample_tensor):
        # With register_buffer, this manual check/move is mostly redundant but kept for safety.
        # However, modifying self.voxel_pts in-place should be avoided if possible.
        pass
        # d_dtype, d_device = sample_tensor.dtype, sample_tensor.device
        # if (self.voxel_pts.dtype != d_dtype) or (str(self.voxel_pts.device) != str(d_device)):
    ...
    def backproject_into_voxel(self, feats_agg, input_mask, intrinsics, extrinsics_inv):
        ...
        for cam in range(self.num_cams):
            ...
            bs, _, h_dim, w_dim = feats_img.size()
    ...
    def calculate_sample_pixel_coords(self, K, v_pts, w_dim, h_dim):
        ...
        # self.eps is float, ensuring safe division
        cam_points = torch.matmul(K[:, :3, :3], v_pts) # [batch_size, 3, n_voxels]

        # Use torch.where or safe division to prevent Inf if z=0 (though clamp handles it later)
        # Added eps inside clamp or division is good.
        pix_coords = cam_points[:, :2, :] / (cam_points[:, 2, :].unsqueeze(1) + self.eps)

        if not torch.all(torch.isfinite(pix_coords)):
            pix_coords = torch.clamp(pix_coords, min=-w_dim*2, max=w_dim*2)
            # If NaNs, zero them out? Clamp handles Inf, but not NaN.
            pix_coords = torch.nan_to_num(pix_coords, nan=0.0)
            
        pix_coords = pix_coords.view(batch_size, 2, self.n_voxels, 1) #[batch_size, 2, n_voxels] -> [batch_size, 2, n_voxels, 1]
    ...
    def project_voxel_into_image(self, voxel_feat, inv_K, extrinsics):
        ...
        for cam in range(self.num_cams):
            ...
            # self.pixel_grid is [1, 2, N]. expand to [b, 2, N] instead of repeat to save memory/dims if possible, but repeat is safe.
            # However, batch_size `b` comes from input. If `pixel_grid` has b=1, repeat works.
            
            # SAFEGUARD: Use expand instead of repeat where possible, but `repeat` creates new memory which might be needed for matmul
            # Using current batch size 'b'
            cam_points = torch.matmul(inv_K[:, cam, :3, :3], self.pixel_grid.repeat(b, 1, 1))
            cam_points = self.depth_grid.repeat(b,1,1,1) * cam_points.view(b, 3, 1, self.num_pix)
            cam_points = torch.cat([cam_points, self.pixel_ones.repeat(b, 1, 1, 1)], dim=1) # [b, 4, n_depthbins, n_pixels]
            ...
            grid = grid.view(b, self.proj_d_bins, self.img_h, self.img_w, 3)            
    ...
    def forward(self, inputs, feats_agg):
        mask = inputs['mask']
        K = inputs['K'].clone()
        scale = 2**(self.fusion_level+1)
        K[:,:,:2,:] /= scale
        inv_K = torch.linalg.inv(K)
        extrinsics = inputs['c2e_extr']
        extrinsics_inv = inputs['e2c_extr']
