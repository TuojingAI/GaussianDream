...
# _DEL_KEYS= ['rgb', 'rgb_context', 'rgb_original', 'rgb_context_original', 'intrinsics', 'extrinsics', 'contexts', 'splitname', 'ego_pose'] 

# _NP_KEYS = ['c2e_extr','K','cam_T_cam','ego_T_ego']
...
def stack_sample(sample):
    ...
    for key in sample[0]:
        ...
        if key in ['idx', 'sample_token','scene_token','frame_context']:
            stacked_sample[key] = sample[0][key]
        # Special handling for vehicle_annotations - keep as list of lists
        elif key == 'vehicle_annotations':
            # Collect vehicle annotations from all cameras (if they exist)
            if all(key in s for s in sample):
                stacked_sample[key] = [s[key] for s in sample]
            # If any sample is missing vehicle_annotations, skip it
        else:
            # Stack torch tensors
...
def inference_transforms(sample, image_shape):
    ...
    """
    # Only do essential transformations for inference
...
def align_3dgs_dataset(sample,frame_id=0):
    """
    ...
    frame_contexts = sample.get('frame_context',[])
    ...
    sample[('color_aug', frame_id)] = aug_images
    sample[('color_org', frame_id)] = org_images

    # for context data
    for idx, frame_offset in enumerate(frame_contexts):
        sample[('color_aug', frame_id+frame_offset)] = aug_contexts[idx]      
        sample[('color_org', frame_id+frame_offset)] = org_contexts[idx]      
    sample['frame_context'] = torch.tensor([frame_id] + frame_contexts,dtype=torch.int64)
...
def stack_4dgs_sample(sample):
    """Stack a sample from multiple sensors"""
    # If there is only one sensor, convert timestamp to array for consistency
    if len(sample) == 1:
    ...
    for key in sample[0]:
        ...
        # Special handling for vehicle_annotations - keep as list of lists
            ...
            if all(key in s for s in sample):
                stacked_sample[key] = [s[key] for s in sample]
            # If any sample is missing vehicle_annotations, skip it
        # Handle scalar values like timestamp (stack into numpy array, preserve int64)
        elif key == 'timestamp':
                ...
                if len(sample[0][key]) > 0:
                    ...
                    # Stack list of numpy arrays
                        ...
                        for i in range(len(sample[0][key])):
                            stacked_sample[key].append(
                                np.stack([s[key][i] for s in sample], 0))
                # If list is empty, keep it as empty list
                else:
                    stacked_sample[key] = []

    return stacked_sample

def align_4dgs_dataset(sample, contexts=None):
    """
    This function reorganize samples to match our trainer configuration.
    """
    K = sample['intrinsics']
    ...
    # Handle optional keys that may not exist
    ...
    # Handle extrinsics if available
    ...
    if contexts and aug_contexts is not None:
        for idx, frame in enumerate(contexts):
            ...
            # Context frame ego poses are handled separately - no need to compute here
