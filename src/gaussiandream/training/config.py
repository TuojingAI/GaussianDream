...
import gaussiandream.models.model as _model
import gaussiandream.models.pi0_config as pi0_config
import gaussiandream.models.pi0_fast as pi0_fast
import gaussiandream.models.tokenizer as _tokenizer
import gaussiandream.policies.aloha_policy as aloha_policy
import gaussiandream.policies.droid_policy as droid_policy
import gaussiandream.policies.libero_policy as libero_policy
import gaussiandream.policies.robocasa_policy as robocasa_policy
import gaussiandream.shared.download as _download
import gaussiandream.shared.normalize as _normalize
import gaussiandream.training.droid_rlds_dataset as droid_rlds_dataset
import gaussiandream.training.depth_transform as depth_transform
import gaussiandream.training.misc.polaris_config as polaris_config
import gaussiandream.training.misc.roboarena_config as roboarena_config
import gaussiandream.training.optimizer as _optimizer
import gaussiandream.training.weight_loaders as weight_loaders
import gaussiandream.transforms as _transforms
...
class DataConfigFactory(abc.ABC):
    ...
    def create_base_config(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        return dataclasses.replace(
            base_config,
            repo_id=repo_id,
            dataset_root=_expand_path(base_config.dataset_root),
            flow_root=_expand_path(base_config.flow_root),
            asset_id=asset_id,
            norm_stats=self._load_norm_stats(epath.Path(_expand_path(self.assets.assets_dir) or assets_dirs), asset_id),
...
class TrainConfig:
    ...
    # Project name.
    project_name: str = "gaussiandream"
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=RobocasaDataConfig(
            ...
            base_config=DataConfig(
                prompt_from_task=True,
                dataset_root="${ROBOCASA_H50_ROOT}",
            ),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
        num_train_steps=30_000,
    ...
    TrainConfig(
        ...
        pytorch_weight_path="${GAUSSIANDREAM_PRETRAINED_DIR}/pi05_libero.safetensors",
        num_train_steps=30_000,
    ),
    ...
    TrainConfig(
        ...
        data=RobocasaDataConfig(
            ...
            base_config=DataConfig(
                prompt_from_task=True,
                dataset_root="${ROBOCASA_OVERFIT_ROOT}",
            ),
        ),
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        checkpoint_base_dir="${GAUSSIANDREAM_CKPT_DIR}",
        batch_size=8,
        num_workers=0,
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            ...
            base_config=DataConfig(
                prompt_from_task=True,
                dataset_root="${LIBERO_DATA_WITH_DEPTH_ROOT}",
                flow_root="${LIBERO_FLOW_ROOT}",
            ),
...
def _alias_config(source_name: str, alias_name: str) -> TrainConfig:
    for config in _CONFIGS:
        if config.name == source_name:
            return dataclasses.replace(config, name=alias_name)
    raise ValueError(f"Cannot create config alias '{alias_name}': source config '{source_name}' not found.")
